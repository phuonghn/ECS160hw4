...
void processValidLine( char* line, struct Slot* hashtable )
{
    ...
    if ( !name )//if this was the empty string
    {
        ...
        else//hit printable or other (apparently emojis aren't detected by isprint)
        {
            ...
            else//during match
            {
                if ( matchPossible && name[n] == headerLine[i] )
                {
                    ...
                    if ( betweenQuotations && n == nameLength && headerLine[i+1] != '\"' )
                    {
                        //we might be about to match
                        //yet failure to finish quotations
                        matchPossible = false;
                    }
                    else if ( betweenQuotations && n == nameLength )
                    {
                        i++;//consume "
                        betweenQuotations = false;//closed quotations successfully
                    }
                }
                else// name[n] != headerLine[i]
                {
...
unsigned long hashCode( unsigned char* str )
{
    ...
    //METHOD 2
    //an alternative method for hashCode:
    /*
     our hashCode for a string will be
     the sum of
     the ints in
     the int array created by
     concatenating all char bits.
     We use an array because
     the chars won't fit in one int.
     */
    ...
    /*
     ...
     if ( str_length <= 0 )
     {
     return 0;//the hashCode of an empty string
     }
     
     //length of the array we'll allocate
     int bits_length = //min number of ints to hold the chars
     (
     str_length + sizeof(int)-1
     )
     / sizeof(int);
     
     int* bits = new int[ bits_length ];
...
struct Slot* findRightfulSlot( char* name, struct Slot* hashtable )
{
    /*if ( strcmp( name, "\"NickApex\"" ) == 0
     //|| strcmp( name, "\"Ardor_Creative\"" ) == 0
     //|| strcmp( name, "\"torithompson13\"" ) == 0
     //|| strcmp( name, "\"urno12\"" ) == 0
     )
     {
